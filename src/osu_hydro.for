...
#include "defs.h"
     ...
     &             .or.(VisSlope > 1d-6)
      ...
      open(921,File='anisotropies_grid.dat',status='REPLACE')
      open(991, file='surface_text.dat', status='replace')
      open(2297, File='viscous_14_moments_evo.dat', status='REPLACE')
...
C###########################################################################################
     ...
     &  NXPhy0,NYPhy0, NXPhy,NYPhy, NX0,NX,NY0,NY,NZ0,NZ)  !PNEW NNEW  related to root finding
      ...
      DO 130 K=NZ0,NZ
      DO 130 J=-NYPhy,NYPhy
      DO 130 I=-NXPhy,NXPhy
        gamma=1.0D0/sqrt(1D0-Vx(I,J,NZ0)**2-Vy(I,J,NZ0)**2)
        xx=I*DX
        yy=J*DY
        xMean=xMean+xx*Ed(I,J,K)*gamma
        yMean=yMean+yy*Ed(I,J,K)*gamma
        xymeanNorm=xymeanNorm+Ed(I,J,K)*gamma
 130  continue
      xMean=xMean/xymeanNorm
      yMean=yMean/xymeanNorm

      ! Now do the eccentricities
      DO 100 K=NZ0,NZ
...
!        yy=J*DY
     ...
     &   +PPI(I,J,K)*U2(I,J,K)*U2(I,J,K)
        ...
        Write(921,'(12G18.8E3)') Time, xx, yy,
     &   (y2-x2)*Ed(I,J,K)*gamma, 2.0*xx*yy*Ed(I,J,K)*gamma,
     &   (y2+x2)*Ed(I,J,K)*gamma, (TXX-TYY), 2.0*TXY, (TXX+TYY),
     &   (TTXX-TTYY), 2.0*TTXY, (TTXX+TTYY)
...
C        STotal=STotal*Time*dx*dy*DT
      ...
      Write(92,'(9G18.8E3)') Time, VAver,
     &   EpsX,EpsP,TEpsP,EpsXMag,EpsPMag,TEpsPMag,Stotal
